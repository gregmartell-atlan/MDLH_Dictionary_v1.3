...
export function useQuery(connectionStatus = null) {
  ...
  const executeQuery = useCallback(async (sql, options = {}) => {
    ...
    if (IS_DEMO) {
      queryLog.info('🎭 Demo mode - executing query against demo data', { sqlPreview: sql.substring(0, 80) });
      setLoading(true);

      // Simulate network delay for realism
      await new Promise(resolve => setTimeout(resolve, 200 + Math.random() * 300));

      // Use the comprehensive demo query engine
      const mockResult = executeDemoQuery(sql);
      queryLog.info('🎭 Demo query result', { rowCount: mockResult.rowCount, columns: mockResult.columns.length });
...
// Demo mode stable functions
const DEMO_PREFLIGHT_RESULT = {
  preflightResult: null,
  loading: false,
  runPreflight: async () => {
    await new Promise(resolve => setTimeout(resolve, 100));
    return { valid: true, message: 'Demo mode: Preflight check simulated', issues: [] };
  },
  clearPreflight: () => {},
};

export function usePreflight() {
  // Demo mode: Return stable mock preflight results
  if (IS_DEMO) {
    return DEMO_PREFLIGHT_RESULT;
  }
...
// Demo mode stable history data
const DEMO_HISTORY_RESULT = {
  history: [
    { id: 'demo-1', sql: 'SELECT * FROM TABLE_ENTITY LIMIT 10', timestamp: Date.now() - 60000, status: 'success' },
    { id: 'demo-2', sql: 'SELECT * FROM COLUMN_ENTITY WHERE TABLENAME = \'FACT_ORDERS\'', timestamp: Date.now() - 120000, status: 'success' },
    { id: 'demo-3', sql: 'SELECT * FROM ATLASGLOSSARYTERM_ENTITY LIMIT 5', timestamp: Date.now() - 180000, status: 'success' },
  ],
  fetchHistory: async () => {},
  loading: false,
};
...
export function useQueryHistory() {
  // Demo mode: Return stable mock query history
  if (IS_DEMO) {
    return DEMO_HISTORY_RESULT;
  }
...
// Demo mode stable function references (defined outside hook to prevent re-creation)
const DEMO_METADATA_FUNCTIONS = {
  fetchDatabases: async () => [DEMO_DATABASE],
  fetchSchemas: async () => [DEMO_SCHEMA],
  fetchTables: async () => DEMO_TABLES.map(t => t.NAME),
  fetchColumns: async (database, schema, table) => {
    const cols = DEMO_COLUMNS.filter(c => c.TABLENAME.toUpperCase() === table?.toUpperCase());
    return cols.map(c => ({ name: c.NAME, dataType: c.DATATYPE }));
  },
  refreshCache: async () => {},
};

const DEMO_METADATA_RESULT = {
  loading: false,
  error: null,
  ...DEMO_METADATA_FUNCTIONS,
};

export function useMetadata(connectionStatus = null) {
  // Demo mode: Return stable mock metadata fetch functions
  if (IS_DEMO) {
    return DEMO_METADATA_RESULT;
  }
...
// Demo mode stable batch validation
const DEMO_BATCH_VALIDATE_FN = async (queries) => {
  await new Promise(resolve => setTimeout(resolve, 200));
  return {
    results: queries?.map((q, i) => ({
      index: i,
      valid: true,
      sql: typeof q === 'string' ? q : q.sql,
      message: 'Demo mode: Validation simulated',
    })) || [],
  };
};

const DEMO_BATCH_RESULT = {
  results: null,
  loading: false,
  error: null,
  validateBatch: DEMO_BATCH_VALIDATE_FN,
};

export function useBatchValidation() {
  // Demo mode: Return stable mock batch validation results
  if (IS_DEMO) {
    return DEMO_BATCH_RESULT;
  }
...
// Demo mode stable explanation functions
const DEMO_EXPLAIN_FN = async (sql) => {
  await new Promise(resolve => setTimeout(resolve, 300));
  return {
    explanation: `This query retrieves data from the MDLH metadata tables. In demo mode, this is a simulated explanation of the SQL: "${sql?.substring(0, 50)}..."`,
    tables: ['TABLE_ENTITY', 'COLUMN_ENTITY'],
    complexity: 'simple',
  };
};

const DEMO_EXPLAIN_RESULT = {
  explanation: null,
  loading: false,
  error: null,
  explainQuery: DEMO_EXPLAIN_FN,
  clearExplanation: () => {},
};

export function useQueryExplanation() {
  // Demo mode: Return stable mock query explanation
  if (IS_DEMO) {
...
// Demo mode stable sample entities
...
export function useSampleEntities() {
  // Demo mode: Return stable pre-populated sample entities
  if (IS_DEMO) {
